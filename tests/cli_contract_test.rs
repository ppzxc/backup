...
fn contract_expectation(
        ...
        || command_path == "backup.schedule"
    {
        ...
        vec![
            ...
            "sh -c curl -fsSL https://github.com/restic/restic/releases/download/v0.16.4/restic_0.16.4_linux_amd64.bz2 -o /tmp/backup-dependency-restic.bz2 && printf '%s  %s\\n' 3d4d43c169a9e28ea76303b1e8b810f0dcede7478555fdaa8959971ad499e324 /tmp/backup-dependency-restic.bz2 > /tmp/backup-dependency-restic.bz2.sha256 && sha256sum -c /tmp/backup-dependency-restic.bz2.sha256 && bunzip2 -c /tmp/backup-dependency-restic.bz2 > <home>/.local/bin/restic && chmod +x <home>/.local/bin/restic && rm -f /tmp/backup-dependency-restic.bz2 /tmp/backup-dependency-restic.bz2.sha256 env=[] timeout=None".into(),
            "which rclone env=[] timeout=None".into(),
            "sh -c curl -fsSL https://downloads.rclone.org/v1.68.1/rclone-v1.68.1-linux-amd64.zip -o /tmp/backup-dependency-rclone.zip && printf '%s  %s\\n' 34f34743b1831523cd2e0aff74447b717e2d62fe1b598e91703899e0c0689568 /tmp/backup-dependency-rclone.zip > /tmp/backup-dependency-rclone.zip.sha256 && sha256sum -c /tmp/backup-dependency-rclone.zip.sha256 && rm -rf /tmp/backup-dependency-rclone && unzip -q /tmp/backup-dependency-rclone.zip -d /tmp/backup-dependency-rclone && find /tmp/backup-dependency-rclone -type f -name rclone -exec cp {} <home>/.local/bin/rclone \\; && chmod +x <home>/.local/bin/rclone && rm -f /tmp/backup-dependency-rclone.zip /tmp/backup-dependency-rclone.zip.sha256 env=[] timeout=None".into(),
            "which resticprofile env=[] timeout=None".into(),
            "sh -c curl -fsSL https://github.com/creativeprojects/resticprofile/releases/download/v0.28.0/resticprofile_0.28.0_linux_amd64.tar.gz -o /tmp/backup-dependency-resticprofile.tar.gz && printf '%s  %s\\n' 8a8b8c611ea86beb9eb095417e851c88e3f1f9e2fda894ea9b2664a94368716a /tmp/backup-dependency-resticprofile.tar.gz > /tmp/backup-dependency-resticprofile.tar.gz.sha256 && sha256sum -c /tmp/backup-dependency-resticprofile.tar.gz.sha256 && rm -rf /tmp/backup-dependency-resticprofile && mkdir -p /tmp/backup-dependency-resticprofile && tar -xzf /tmp/backup-dependency-resticprofile.tar.gz -C /tmp/backup-dependency-resticprofile && cp /tmp/backup-dependency-resticprofile/resticprofile <home>/.local/bin/resticprofile && chmod +x <home>/.local/bin/resticprofile && rm -f /tmp/backup-dependency-resticprofile.tar.gz /tmp/backup-dependency-resticprofile.tar.gz.sha256 env=[] timeout=None".into(),
        ]
