...
fn test_language_detect_from_falls_back_to_lang_when_lc_all_absent() {
    ...
    assert_eq!(
        lang,
        Language::Ko,
        "LC_ALL 없을 때 LANG=ko면 Korean이어야 합니다"
    );
}
...
fn test_language_detect_from_lc_all_ko_wins_over_lang_en() {
    ...
    assert_eq!(
        lang,
        Language::Ko,
        "LC_ALL=ko이면 LANG=en이어도 Korean이어야 합니다"
    );
}
...
fn test_language_detect_from_empty_lc_all_falls_back_to_lang() {
    ...
    assert_eq!(
        lang,
        Language::Ko,
        "LC_ALL='' 일 때 LANG=ko면 Korean이어야 합니다"
    );
}
...
fn test_cli_help_korean_contains_only_korean() {
    ...
    assert!(
        h.cmd_setup.contains("마법사"),
        "setup 도움말이 한국어여야 합니다"
    );
    assert!(
        h.cmd_run.contains("파이프라인"),
        "run 도움말이 한국어여야 합니다"
    );
    assert!(
        h.cmd_doctor.contains("진단"),
        "doctor 도움말이 한국어여야 합니다"
    );
    assert!(
        h.cmd_schedule.contains("타이머"),
        "schedule 도움말이 한국어여야 합니다"
    );
    assert!(
        h.cmd_restore.contains("복구"),
        "restore 도움말이 한국어여야 합니다"
    );
    assert!(
        h.cmd_snapshots.contains("스냅샷"),
        "snapshots 도움말이 한국어여야 합니다"
    );
    assert!(
        h.cmd_status.contains("운영 상태"),
        "status 도움말이 한국어여야 합니다"
    );
    assert!(
        h.cmd_update.contains("업데이트"),
        "update 도움말이 한국어여야 합니다"
    );
    assert!(
        h.cmd_version.contains("버전"),
        "version 도움말이 한국어여야 합니다"
    );
    assert!(
        h.cmd_uninstall.contains("삭제"),
        "uninstall 도움말이 한국어여야 합니다"
    );
}
...
fn test_cli_help_english_contains_only_english() {
    ...
    assert!(
        h.cmd_doctor.contains("diagnostics"),
        "doctor help must be English"
    );
    assert!(
        h.cmd_schedule.contains("scheduler"),
        "schedule help must be English"
    );
    assert!(
        h.cmd_restore.contains("Restore"),
        "restore help must be English"
    );
    assert!(
        h.cmd_snapshots.contains("snapshots"),
        "snapshots help must be English"
    );
    assert!(
        h.cmd_status.contains("operational"),
        "status help must be English"
    );
    assert!(
        h.cmd_update.contains("Self-update"),
        "update help must be English"
    );
    assert!(
        h.cmd_version.contains("version"),
        "version help must be English"
    );
    assert!(
        h.cmd_uninstall.contains("Uninstall"),
        "uninstall help must be English"
    );
}
...
fn test_cli_help_no_korean_mode_with_english_keywords_all_fields() {
    ...
    let fields: &[(&str, &str)] = &[
        ...
        ("cmd_report_environment", h.cmd_report_environment),
        ("cmd_report_time_sync", h.cmd_report_time_sync),
        ("cmd_report_restore_drill", h.cmd_report_restore_drill),
        ("opt_report_file", h.opt_report_file),
    ...
    for (field_name, value) in fields {
        for kw in &en_keywords {
            let is_proper = proper_nouns
                .iter()
                .any(|n| kw.to_lowercase().starts_with(&n.to_lowercase()));
