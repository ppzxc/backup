...
fn test_e2e_db_streaming_mariadb_latest() {
    ...
    let node = mariadb_image
        .start()
        .expect("Failed to start MariaDB 10.11 container");
    let host_port = node
    ...
    if let Ok(dump_res) = dump_out {
        ...
        if !dump_str.is_empty() {
            assert!(
                dump_str.contains("users"),
                "Dump stream must contain created users table!"
            );
        }
    }
...
fn test_e2e_db_streaming_mariadb_legacy() {
    ...
    let node = mariadb_image
        .start()
        .expect("Failed to start MariaDB 10.6 container");
    let host_port = node
        .get_host_port_ipv4(3306)
        .expect("Failed to get MariaDB port");
    assert!(host_port > 0);
...
fn test_e2e_db_streaming_postgres_latest() {
    ...
    let node = pg_image
        .start()
        .expect("Failed to start Postgres 16 container");
    let host_port = node
    ...
    if let Ok(dump_res) = dump_out {
        ...
        if !dump_str.is_empty() {
            assert!(
                dump_str.contains("audit_events"),
                "Postgres dump stream must contain audit_events table!"
            );
        }
    }
