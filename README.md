...
본 스크립트는 한국인터넷진흥원(KISA)의 **ISMS-P 인증 기준(특히 2.9.3 백업 및 복구관리, 2.12 재해복구)** 요건을 충족하도록 설계되었습니다. 보안 심사 시 제출해야 하는 백업 정책 설정, 권한 통제, 복구 훈련 이행 등 주요 항목에 대해 본 도구가 제공하는 기능과 증적 증빙 매핑은 다음과 같습니다.

### 📋 ISMS-P 백업 요건 대비 체크리스트 및 구현 내용

| ISMS-P 인증 기준 | 인증 요구사항 및 통제 항목 | 본 스크립트 구현 내용 및 기술적 조치 | 확보 가능한 심사 증적 서류 |
| :--- | :--- | :--- | :--- |
| **2.9.3 백업 및 복구관리** | **주요 데이터 백업 정책 수립**<br>주요 정보시스템 데이터, 시스템 설정 파일, 보안 감사 로그 등의 정기 백업 정책 수립 및 스케줄링 가동 | • `/etc`(중요 설정) 및 `/var/log`(감사 로그) 경로 기본 백업 강제<br>• `schedule enable` 명령을 통해 systemd timer 백업 스케줄러 자동 가동 (매일 새벽 2시) | • `backup.env` 백업 정책 파일<br>• systemd timer 가동 상태 화면 및 `status` 스크린샷 |
| **2.9.3 백업 및 복구관리** | **백업 암호화 및 보존 주기 제어**<br>백업 데이터의 보존 기간 관리 및 원격 전송/저장 시 암호화 통제 | • AES-256 기반 저장소 자체 암호화 및 마스터 비밀번호 설정 강제<br>• `keep-daily`(7일), `keep-weekly`(4주), `keep-monthly`(12개월) 법적 보존 연한 설정 자동화 및 Forget/Prune 파이프라인 수행 | • `audit_report.txt` 정책 요약본<br>• `restic snapshots` 보존 주기 이력 로그 |
| **2.9.3 백업 및 복구관리** | **백업 환경 접근 통제**<br>백업 파일 및 설정 정보에 대한 비인가자 접근 통제 | • 백업 설정 디렉터리(`700`) 및 백업 자격 증명 환경 변수 파일(`backup.env`, `600`)의 권한 위반 여부 자동 탐지 및 권한 강제 | • `status` 명령을 통한 디렉터리/파일 권한 무결성 점검 결과 |
| **2.9.3 백업 및 복구관리**<br>**2.12 재해복구** | **정기 복구 모의훈련 수행**<br>백업 데이터의 정합성 검증 및 재해 복구 훈련을 최소 연 1회 이상 수행하고 보고서 작성 | • `backup.sh audit --restore-drill --report`를 통한 실제 모의 훈련 수행 자동화<br>• 원격 저장소 스냅샷 임시 복원 및 DB 테이블 레코드 카운트 등 정합성 검리 쿼리 수행 | • **`restore_drill_report_YYYYMMDD.html`** (A4 출력용 공식 모의훈련 보고서)<br>• `.json`, `.txt` 이행 서류 |
| **2.9.3 백업 및 복구관리** | **백업 가동 상태 모니터링**<br>백업 수행 결과에 대한 모니터링, 오류 탐지 및 이력 관리 | • 백업 성공 및 실패 시 Slack, Discord, Custom Webhook 연동 알림<br>• `backup.sh audit --daily --report`를 통해 매일 새벽 백업 무결성 점검 보고서 생성 | • **`daily_backup_audit_report_YYYYMMDD.html`** (A4 출력용 공식 일일 백업 보고서)<br>• 메신저 웹훅 알림 메시지 이력 |
