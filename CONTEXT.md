...
* **비고**: 모든 CLI command의 구조화된 진단 로그는 시스템 로그 sink(Journald -> Syslog -> Rotating File)로만 전달하고 터미널의 `stdout`·`stderr`에 직접 출력하지 않습니다. `stdout`과 `stderr`에는 CLI User Notice 또는 명령 실패 진단만 포함되어 구조화 로그와 격리됩니다. 3단계 폴백 파이프라인(1. Journald socket `/run/systemd/journal/socket`, 2. Syslog socket `/dev/log`, 3. 회전 로그 파일 `/var/log/backup/backup.log` 순차 적용)을 통해 어떤 실행 환경에서도 진단 로그를 안전하게 수집하며, 비밀값(`SecretString`, `password`, `access_key`, `secret_key`, `token` 등)은 `***MASKED***`로 자동 은닉합니다. 대화형 설정 위자드(`backup setup`)는 사용자 인터페이스를 `stdout`으로 유지하고 구조화 로그를 터미널에 출력하지 않아 TUI UI 깨짐을 방지합니다. Logging precedence는 명시적 CLI option(`-q` 또는 `-v`) -> `BACKUP_LOG` -> `RUST_LOG` -> 기본 `info` 순서입니다. `--quiet`와 모든 `-v` 조합은 parser 단계에서 exit code `2`로 거부합니다. `-v` 개수는 `0`, `1`, `2`, `3 이상`을 각각 `info`, `debug`, `trace`, `trace`로 해석합니다. 이 필터는 시스템 로그에만 적용하고 CLI User Notice의 표시 여부에는 적용하지 않습니다. 전역 `--log-file <path>`가 지정되면 해당 경로를 우선 사용하고 부모 디렉터리 `700`, 파일 `600`을 강제합니다. 생성·open·권한 설정 실패는 fallback하지 않고 command 실행 전 exit code `1`로 전파하며, 미지정 시에만 Journald -> Syslog -> 기본 파일 fallback을 사용합니다.

### 14-1. CLI User Notice (CLI 사용자 알림)
* **설명**: 사용자가 명령의 진행·완료·취소 상태를 이해하기 위해 직접 보는 비구조화 메시지.
* **비고**: CLI User Notice는 비대화형 실행에서 `CommandOutcome.stdout`으로, 대화형 Setup Wizard에서는 TUI 전용 렌더링 경로로 출력하며, `tracing` 구조화 진단 로그로 표현하지 않습니다. 백엔드 연결·저장소 초기화 같은 진행 알림과 성공·취소 알림을 포함하며, 사용자의 다음 입력·판단에 필요한 경고·오류 안내도 severity와 관계없이 포함합니다. 비대화형 명령이 실패해도 이미 발생한 진행 알림은 stdout에 보존하고 최종 실패 진단은 stderr에 둡니다. JSON·CSV 등 기계 판독 출력 모드에서는 결과 stdout을 오염시키지 않도록 비필수 알림을 생략합니다.
...
* **비고**: CLI Command Contract는 하나의 authoritative CLI schema를 기준으로 입력 option의 해석, Backup Pipeline 외부 adapter 호출 순서, 성공·실패 결과, stdout/stderr, 생성 산출물, 외부 adapter 동작을 함께 설명하며, command 구현의 내부 구조와 분리해 검증합니다. Command의 결과는 CLI User Notice·stdout data, stderr failure diagnostics, exit status, 생성 artifact와 외부 상태 변화를 포함하는 구조화된 command outcome으로 관찰하며, System Diagnostic Logging은 이 outcome의 stdout/stderr에 포함하지 않습니다. 사람이 읽는 출력에서는 CLI User Notice를 stdout에 포함하되 JSON·CSV 등 기계 판독 모드에서는 stdout을 결과 데이터만으로 제한하고 비필수 알림을 생략합니다. 기본 Backup Pipeline 순서는 Database Stream(선택) -> Primary Backend Adapter -> Secondary Backend Adapter(선택) -> Retention -> Backup Execution Report입니다.
